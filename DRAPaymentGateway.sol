...
contract DRAPaymentGateway is DRAWithdrawable, EIP712 {
    ...
    function payment(uint256 channelId, uint256 invoiceNo, uint256 deadline, IERC20 tokenContract, uint256 amount, bytes memory signature) external {
        ...
        require(orderSigner != address(0) && cashier != address(0), "DRAPaymentGateway: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "DRAPaymentGateway: order already paid");
